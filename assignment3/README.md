# FUTURE BUSINESS MAJORS – MILESTONE 3

---
## Reproducibility Assessment of Code Clone Detection Tools


<div>
  <span style="float:left;">CMPT-470</span>
  <span style="float:right;">2026-02-02</span>
  <div style="clear: both;"></div>
</div>

---


### Group Members

| Name                  | ID     |
|:-----------------------:|:--------:|
| Kylen Morris          | GTH842 |
| Darian Rowat          | DDR271 |
| Rylan Curtis          | ODY435 |
| Volodymyr Akulov      | ONC736 |
| Kyran McKnight-Woods  | ZOE821 |

---

### Assigned Papers

|Paper|      
|:-----------------------:|
| [Agec](https://ieeexplore.ieee.org/document/6613854)|
| [Code Clone Detection Techniques](https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=10918947)| 
| [Hyclone](https://arxiv.org/abs/2508.01357)| 
| [StarChat](https://huggingface.co/HuggingFaceH4/starchat-beta)| 
| [XCode](https://dl.acm.org/doi/pdf/10.1145/3506696)| 
| [Zhao](https://arxiv.org/abs/2502.19219)|

---

##### Team Name: Future Business Majors
##### Milestone: Reproducibility Assessment of Code Clone Detection Tools
|Name|Contribution (%)|      
|:-----------------------:|:---:|
|Kylen Morris|21.25|
|Darian Rowat|21.25|
|Rylan Curtis|15|
|Vlad Akulov|21.25| 
|Kyran McKnight-Woods|21.25|

Usage of AI Tools by each member:
|Name|AI Usage|      
|:-----------------------:|:---:|
|Kylen Morris|Used AI to refactor Starchat script.|
|Darian Rowat|Once prompted ChatGPT to find tool source code. It did not work.|
|Rylan Curtis|No AI used.|
|Vlad Akulov|Wrote shell script for Agec.| 
|Kyran McKnight-Woods|Wrote Python script for LLM smoketesting.|
